...
def filter_spells(spells_collection, params):
    ...
    if 'name' in params:
        filter_params['name'] = {'$regex': '.*{}.*'.format(params['name'])}
...
async def get_filtered_spells(spells_collection, params):
    filtered_query = filter_spells(spells_collection, params)
    pages = 1
    if 'page' in params:
